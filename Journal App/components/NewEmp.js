...
export default class NewEmp extends React.Component {
	constructor(props) {
		super(props)
		this.state = {
			full_name: '',
	...
	callBackend = async () => {
		...
		if (this.state.bcolours.every((element) => {return(element == 'black')})) {
			...
			let payload = { email: this.state.email, pass: this.state.pass, name: this.state.full_name, emp_email: this.state.empEmail}
			try {
				var response = await axios.post(this.state.ip + "NewEmp", payload)
